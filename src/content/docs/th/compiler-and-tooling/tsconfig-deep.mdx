---
title: 'tsconfig, Deep'
description: 'compiler option ที่เปลี่ยนความปลอดภัยและ output ของคุณจริง ๆ — strict และครอบครัวของตัวเอง, target, module และ flag ข้าง ๆ strict ที่ควรเปิด'
sidebar:
  order: 2
---
...
เปิด `strict: true` ตั้งแต่วันแรก — flag นี้รวมชุดของ check ไว้ และแค่ `strictNullChecks` ตัวเดียวก็กำจัด class ของ JavaScript crash ที่พบบ่อยที่สุดได้ ตั้ง `target`, `module` และ `lib` ให้ตรงกับ runtime ของคุณ แล้วเพิ่ม `noUncheckedIndexedAccess` กับ `exactOptionalPropertyTypes` เพื่อจับ bug ที่ strict ไม่จับ
</Callout>
