...
เมื่อไม่มีอะไรให้ infer ได้เลย `T` จะไม่กลายเป็น error — มันกลายเป็น `unknown` `new Stack()` เฉย ๆ ให้ `Stack<unknown>` และเพราะ `unknown` ในตำแหน่ง *parameter* ยอมรับทุกอย่าง push ทั้งสองด้านล่างจึง type-check ผ่าน:
