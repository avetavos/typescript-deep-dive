...
conditional type `T extends U ? X : Y` คือ if-statement ในระดับ type ใช้การทดสอบ subset แบบเดียวกับ `extends` ที่อื่น พฤติกรรมเด่นของตัวเองคือ **distribution**: naked type parameter ที่ตรวจกับ union จะรันทีละ member แล้ว union ผลลัพธ์ (นั่นคือวิธีที่ `Exclude` และ `Extract` ทำงาน) ห่อทั้งสองฝั่งเป็น `[T] extends [U]` เพื่อตรวจ union ทั้งก้อนและปิด distribution
</Callout>
