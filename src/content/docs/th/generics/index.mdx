...
generic คือ **function ในระดับ type** — มันรับ type เป็น input แล้วผลิต type เป็น output `identity<T>` บอกว่า "สำหรับ type `T` ใด ๆ ก็ตาม รับ `T` แล้วคืน `T`" อันนี้เรียกว่า *parametric polymorphism* — implementation เดียว ที่ parameterize ด้วย type
...
Generics ทำให้ implementation เดียวทำงานได้กับหลาย type **โดยไม่เสียข้อมูล type** — ซึ่งเป็นสิ่งที่ `any` ทิ้งไป type parameter อย่าง `T` คือตัวยึดที่ผู้เรียกหรือ inference เติมค่าให้ ดังนั้น `identity<T>` จึงคืน type เดียวกับที่รับเข้ามาเป๊ะ มอง generic เป็น function ที่รันในระดับ type
</Callout>
