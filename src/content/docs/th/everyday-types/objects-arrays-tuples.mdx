...
## object type และ modifier ของมัน
...
- **`readonly`** บล็อกการ reassign property มันเป็น compile-time เท่านั้น (runtime ไม่ได้กันการ mutate) แต่มันสื่อและ enforce เจตนา
...
เมื่อยังไม่รู้ key ล่วงหน้า **index signature** type มันแบบ uniform:
...
tuple ขับเคลื่อน pattern อย่าง return ของ `useState` ใน React (`[value, setter]`) และพิกัดแบบมีชื่อกลาย ๆ มันยังรองรับ **label** (เป็น documentation ไม่มีผล runtime) และ rest element:
