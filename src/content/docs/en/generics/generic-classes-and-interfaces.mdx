...
When nothing lets it infer at all, `T` doesn't become an error — it becomes `unknown`. `new Stack()` alone gives you a `Stack<unknown>`, and since `unknown` in a *parameter* position accepts anything, both pushes below type-check:
