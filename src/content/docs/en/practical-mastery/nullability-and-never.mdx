...
Turn on `strictNullChecks` so `null` and `undefined` must be handled, not ignored. Use `?.` and `??` (which the types understand) for ergonomic null handling, and treat every non-null `!` as an overridden safety check. Model failure with **Result** discriminated unions instead of invisible throws, and use the `never` **exhaustiveness** trick so adding a new union variant becomes a compile error, not a silent gap.
</Callout>
